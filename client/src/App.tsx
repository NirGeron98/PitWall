...
type View = 'races' | 'drivers' | 'standings' | 'profile' | 'compare' | 'favorites';
...
const DriverProfileScreen: React.FC<{
  ...
  if (!driver) {
    return <div style={{ padding: '64px', color: 'var(--text-secondary)' }}>Driver not found.</div>;
  }
...
function AppShell() {
  ...
  const authMode: 'login' | 'register' = location.pathname.startsWith('/signup') ? 'register' : 'login';
  ...
  const [selectedDriverForTelemetry, setSelectedDriverForTelemetry] = useState<Driver | null>(null);
  const [isTelemetryOpen, setIsTelemetryOpen] = useState(false);
  const [compareSelection, setCompareSelection] = useState<string[]>([]);
  const [compareStats, setCompareStats] = useState<Record<string, DriverSeasonStats | null>>({});
  ...
  const toggleCompare = (driverNum: string) => {
    setCompareSelection(prev => {
      if (prev.includes(driverNum)) return prev.filter(id => id !== driverNum);
      const max = 4;
  ...
  const handleLoginSubmit = async (email: string, password: string) => {
    ...
    try {
      ...
      setAuthStatus(null);
      setAuthError('Unable to authenticate. Check your credentials and try again.');
      throw err;
    }
  };
    ...
    <div className="app-shell">
      ...
      <main className="container app-main">
        <div className="control-bar flex-row justify-between">
          <div className="flex-row items-center" style={{ flexWrap: 'wrap', gap: '8px' }}>
            <h1 className="page-title">
              {activeView === 'races' && 'Race Calendar'}
              {activeView === 'drivers' && 'Driver Roster'}
              {activeView === 'standings' && 'Standings'}
              {activeView === 'favorites' && 'My Favorites'}
              {activeView === 'compare' && 'Driver Comparison'}
              {activeView === 'profile' && 'Driver Profile'}
            </h1>
          </div>
        </div>
        ...
        <div className="animate-enter">
          {loading ? (
            <div className={activeView === 'drivers' ? 'drivers-grid' : 'races-grid'}>
                ...
                <div className="races-grid">
                  {filteredRaces.map(race => (
                    <RaceCard
                      ...
                      race={race}
                      onClick={(r) => { setSelectedRace(r); setIsRaceModalOpen(true); }}
                    />
                  ))}
            ...
            />
                        ...
                        <div className="team-drivers">
                          {group.drivers.map(driver => (
                            <DriverCard
                              ...
                              onClick={(d) => navigate(`/drivers/${d.DriverNumber}`)}
              ...
              <Route
                ...
                element={
                  <FavoritesView
                    ...
                    onToggleFavorite={handleToggleFavorite}
                    onSelectDriver={(driver) => navigate(`/drivers/${driver.DriverNumber}`)}
                    onOpenTelemetry={handleTelemetryOpen}
                  />
                }
              />
              ...
              <Route
                ...
                element={
                  <DriverCompare
                    ...
                    onClear={() => { setCompareSelection([]); setCompareStats({}); }}
                  />
                }
              />
              ...
              <Route
                path="/standings"
                element={
                  <StandingsView onDriverSelect={(driverNumber) => navigate(`/drivers/${driverNumber}`)} />
                }
              />
            </Routes>
          )}
